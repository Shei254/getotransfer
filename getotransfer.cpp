#include "getotransfer.h"

/**
 * This is the main program entry point
 * usage:
 * getotransfer <server root directory> <port to listen on for incoming connections> <telnet mode for use with telnet client, default = false>
 */
int main(int argc, char** argv) {
    ...
    if (argc < 2) {
        std::cout << "Usage: getotransfer <dir> <port> [telnetmode=no], using default dir '" << dir << "' , port " << port << std::endl;
    } else {
        switch (argc) {
            case 4:
